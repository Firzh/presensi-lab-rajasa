...
CREATE DATABASE IF NOT EXISTS `sistem_presensi_siswa_qr`
...
CREATE TRIGGER `trg_rombel_prevent_null_tahun_ajaran_before_update`
BEFORE UPDATE ON `rombel`
FOR EACH ROW
BEGIN
  IF NEW.`tahun_ajaran_id` IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'tahun_ajaran_id rombel tidak boleh kosong';
  END IF;
END$$
...
CREATE TABLE `users` (
  ...
  `user_type` ENUM('siswa','guru','staff','admin','super_admin','intern') NOT NULL,
  ...
  CONSTRAINT `fk_users_guru_staff`
...
CREATE TRIGGER `trg_users_validate_identity_before_insert`
BEFORE INSERT ON `users`
FOR EACH ROW
BEGIN
  IF NEW.`user_type` = 'siswa' THEN
    IF NEW.`siswa_id` IS NULL OR NEW.`guru_id` IS NOT NULL THEN
      SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'User siswa wajib memiliki siswa_id dan tidak boleh memiliki guru_id';
    END IF;
  END IF;

  IF NEW.`user_type` IN ('guru','staff','admin','super_admin') THEN
    IF NEW.`guru_id` IS NULL OR NEW.`siswa_id` IS NOT NULL THEN
      SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'User guru/staff/admin wajib memiliki guru_id dan tidak boleh memiliki siswa_id';
    END IF;
  END IF;

  IF NEW.`user_type` = 'intern' THEN
    IF NEW.`siswa_id` IS NOT NULL THEN
      SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'User intern tidak boleh memakai siswa_id';
    END IF;
  END IF;
END$$

CREATE TRIGGER `trg_users_validate_identity_before_update`
...
CREATE TRIGGER `trg_presensi_sesi_validate_mode_before_insert`
BEFORE INSERT ON `presensi_sesi`
FOR EACH ROW
BEGIN
  IF NEW.`mode_presensi` = 'rombel' AND NEW.`rombel_id` IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Mode rombel wajib memiliki rombel_id';
  END IF;

  IF NEW.`mode_presensi` = 'piket' AND NEW.`rombel_id` IS NOT NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Mode piket tidak boleh memiliki rombel_id';
  END IF;
END$$

CREATE TRIGGER `trg_presensi_sesi_validate_mode_before_update`
BEFORE UPDATE ON `presensi_sesi`
FOR EACH ROW
BEGIN
  IF NEW.`mode_presensi` = 'rombel' AND NEW.`rombel_id` IS NULL THEN
    SIGNAL SQLSTATE '45000'
