 ...
 * app.jsx
 *
 * Root application component — teacher (guru) branch.
 *
 * Auth flow:
 *   1. Baca token + user dari localStorage (STORAGE_KEYS)
 *   2. Jika ada & user_type = guru → DashboardGuru
 *   3. Jika tidak ada → halaman login (dari alfy/landing-page via app.css)
 *
 * Dev routes (/dev/*) tetap dipertahankan untuk keperluan testing.
 *
...
// ─── Dev routes (preserved for testing) ──────────────────────────────────

const path = window.location.pathname

if (path === '/dev/scan')              { /* handled below */ }
if (path === '/dev/import')            { /* handled below */ }
if (path === '/dev/attendance-audit')  { /* handled below */ }

// ─── Login page (reuse alfy landing-page design via app.css) ──────────────
  ...
  return (
    <div style={{
      ...
      <p style={{ fontWeight: 600, color: '#1e293b', margin: 0 }}>
        Dashboard untuk role <code>{role}</code> belum tersedia di branch ini.
      </p>
...
export function App() {
  ...
  const [theme, setTheme] = useState(() =>
    appStorage.getRaw(THEME_KEY) || 'light'
  )
  const [authUser,     setAuthUser]     = useState(() =>
    appStorage.getJSON(STORAGE_KEYS.AUTH_USER, null)
  )
